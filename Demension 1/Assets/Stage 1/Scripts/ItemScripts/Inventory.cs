...
{
    public List<Itemver> inventory = new List<Itemver>();
    // 인벤토리를 리스트로 만듭니다.
    private itemDatabase db;
    // 아이템 데이터베이스는 db로 축약해서 사용합니다.

    public int slotX, slotY;    // 인벤토리 가로 세로 속성 설정 위한 변수
    public List<Itemver> slots = new List<Itemver>(); // 인벤토리 속성 변수
    ...
    void Start()
    {
        ...
        {
            slots.Add(new Itemver());
            // 아이템 슬롯칸에 빈 오브젝트 추가하기
            inventory.Add(new Itemver());
            // 인벤토리에 추가
        }
            ...
            for (int i = 0; i < slotX; i++)
            {
                ...
                if (slots[k].itemName != null)
                {
                    ...
                    // 만약 마우스가 해당 인벤토리 창-버튼-위로 올라온다면,
                    {
                        ...
                        {
                            if (inventory[k].itemType == Itemver.ItemType.Use)
